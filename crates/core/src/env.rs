...
static ENV_VARS: Lazy<HashMap<String, String>> = Lazy::new(|| {
    ...
    if let Some(window) = web_sys::window() {
        if let Ok(env_obj) = js_sys::Reflect::get(&window, &"__ENV__".into()) {
            if let Ok(entries) = js_sys::Object::entries(&env_obj.into()) {
                for i in 0..entries.length() {
                    if let Ok(entry) = entries.get(i) {
                        if let Ok(entry_array) = js_sys::Array::from(&entry) {
...
pub fn env_or(key: &str, default: &str) -> String {
    ENV_VARS.get(key).cloned().unwrap_or_else(|| default.to_string())
}
        ...
        Ok(Config {
            ...
            features: Features {
                maintenance_mode: env("MAINTENANCE_MODE")
                    .map(|v| v == "true")
                    .unwrap_or(false),
                new_feature_flag: env("FEATURE_NEW_UI")
                    .map(|v| v == "true")
                    .unwrap_or(false),
...
impl FeatureFlags {
    pub fn is_enabled(feature: &str) -> bool {
        match feature {
            "maintenance_mode" => config().map(|c| c.features.maintenance_mode).unwrap_or(false),
            "new_ui" => config().map(|c| c.features.new_feature_flag).unwrap_or(false),
...
pub fn with_env<T>(
    production: impl FnOnce() -> T,
    development: impl FnOnce() -> T,
) -> T {
    if is_production() {
        production()
    } else {
